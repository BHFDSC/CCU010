...
dbutils.notebook.run("Helper_functions/skinny", 9000)
dbutils.notebook.run("Helper_functions/functions", 9000)
...
def skinny_unassembled(_hes_apc, _hes_ae, _hes_op, _gdppr, _deaths, _deaths_dates_invalid, _print_id=False):
  ...
  # ------------------------------------------------------------------------------------
  _dss_mapping_snomed_ethnic = spark.table('.gdppr_ethnicity_mappings')\
...
db = ''
dbc = f'{db}_collab'
unassembled = spark.table(f'{dbc}.{proj}_tmp_skinny_unassembled')
display(unassembled)
...
def skinny_assembled(_unassembled, _overall_censor_date=None, _individual_censor_dates=None, _prioritise_primary_care=1):
  ...
  # FROM (
  #   SELECT ETHNICITY_CODE, ETHNICITY_DESCRIPTION FROM .hesf_ethnicity
  #   UNION ALL
  #   SELECT Value as ETHNICITY_CODE, Label as ETHNICITY_DESCRIPTION FROM .gdppr_ethnicity WHERE Value not in (SELECT ETHNICITY_CODE FROM FROM .hesf_ethnicity)) """)
...
# MAGIC %md # 6 Load Skinny Assmebled Data
...
db = ''
dbc = f'{db}_collab'
assembled = spark.table(f'{dbc}.{proj}_tmp_skinny_assembled')
# display(skinny)
...
assembled.createOrReplaceGlobalTempView(f"{proj}_final_skinny_assembled")
drop_table(f"{proj}_final_skinny_assembled", if_exists=True)
create_table(f"{proj}_final_skinny_assembled", select_sql_script=f"SELECT * FROM global_temp.{proj}_final_skinny_assembled")